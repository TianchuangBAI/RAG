    ...
    while True:
        try:
            ...
            if query.lower() in ['help', '帮助', 'h']:
                print("\n帮助信息:")
                print("- 直接输入TCM相关问题即可获得答案")
                print("- 系统会自动检索相关TCM文档并生成回答")
                print("- 使用LoRA微调后的模型，具有更好的TCM领域适应性")
                print("- 输入 'quit' 或 'exit' 退出系统")
            ...
            if result['success']:
                ...
                print(f"❌ 生成TCM答案时出现错误: {result['answer']}")
...
def batch_mode(rag_system):
    """批量处理模式"""
    QUERY_FILE = "query/query_TCM.txt"  # 查询文件路径
    ...
    print(f"📖 共读取到 {len(queries)} 个TCM查询")
    print()
    ...
    for i, query in enumerate(queries, 1):
        ...
        try:
            ...
            if result['success']:
                ...
                print(f"   ❌ 生成TCM答案失败: {result['answer']}")
            
        except Exception as e:
            print(f"   ❌ 处理TCM查询时出现错误: {e}")
            query_result = {
                'query': query,
                'success': False,
                'answer': f"处理错误: {e}",
    ...
    try:
        with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
            f.write("TCM LoRA微调模型RAG系统批量处理结果\n")
            f.write("="*80 + "\n\n")
...
def main():
    """主函数 - 用户选择模式"""
    # 配置参数 - 使用TCM相关的路径
    PDF_FOLDER = "knowledge_base_TCM"  # PDF文件夹路径
    BASE_MODEL_PATH = "./Qwen/Qwen3-14B"  # 基础模型路径
    LORA_CHECKPOINT_PATH = "output/checkpoint/TCM-checkpoint"   # LoRA checkpoint路径
